...
interface IAppliedSeedRow extends RowDataPacket {
  module_name: string;
  version: string;
}
...
export class SeedRepository {
  ...
  public async ensureSeedTable(): Promise<void> {
    await this.executor.execute(`
      CREATE TABLE IF NOT EXISTS schema_seeds (
        ...
        UNIQUE KEY uq_schema_seeds_module_version (module_name, version)
      );
    `);
  }

  public async getAppliedKeys(): Promise<Set<string>> {
    const rows = await this.executor.query<IAppliedSeedRow[]>(
      "SELECT module_name, version FROM schema_seeds ORDER BY module_name ASC, version ASC"
    );
  ...
  public async markAsApplied(moduleName: string, version: string): Promise<void> {
    await this.executor.execute(
      "INSERT INTO schema_seeds (module_name, version) VALUES (?, ?)",
      [moduleName, version]
    );
  }
}